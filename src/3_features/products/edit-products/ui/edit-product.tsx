...
import { useForm, useFranchise, useSubgroup, useAutoManager, useDirection, useEconomist, useGroup, useSeason, useSubdivision, useSubsubgroup, useTeam, useTypeSender, } from "../model";
...
export const EditProduct: FC<Props> = ({ product, productName, productCode,  productLabels, onClose, onSuccess }) => {
  ...
  const handleValueChange = (fieldOnChange: (value: any) => void, fieldName: keyof FormValues) => {
    return (value: string[]) => {
      ...
      const filteredValue = value.filter((val: any) => 
        val !== '0' && 
        val !== 0 && 
        val !== '' && 
        val !== null && 
                      ...
                      <CardContent className="flex flex-col gap-3">
                        <FormField
                          ...
                          render={({field}) => {
                            ...
                            return (
                              <FormItem>
                                <FormLabel  className={hasError ? "text-destructive" : ""}>Структура продаж</FormLabel>
                                <FormControl>
                                <MultiSelect
                                maxCount={1}
                                value={Array.isArray(field.value) ? field.value.map(String) : []}
                                  ...
                                  onValueChange={handleValueChange(field.onChange, 'groupFranchise')}
                                  externalLabels={productLabels?.groupsFranchise ?? []}
                                ...
                                {hasError && (
          <p className="text-[12px] text-destructive">Обязательно для заполнения</p>
        )}
                              </FormItem>
                            );
                          }}
                        ...
                        <FormField
                          ...
                          render={({field}) => {
                            ...
                            return (
                              <FormItem>
                                <FormLabel  className={hasError ? "text-destructive" : ""}>Группа</FormLabel>
                                <FormControl>
                                <MultiSelect
                                maxCount={1}
                                value={Array.isArray(field.value) ? field.value.map(String) : []}
                                  ...
                                  onValueChange={handleValueChange(field.onChange, 'idGroupMain')}
                                  externalLabels={productLabels?.groupsMain ?? []}
                                  defaultValue={Array.isArray(field.value) ? field.value.map(String) : []}
          ...
          <p className="text-[12px] text-destructive">Обязательно для заполнения</p>
                        ...
                        <FormField
                          ...
                          render={({field}) => {
                            ...
                            return (
                              <FormItem>
                                <FormLabel className={hasError ? "text-destructive" : ""}>Подгруппа</FormLabel>
                                <FormControl>
                                <MultiSelect
                                maxCount={1}
                                value={Array.isArray(field.value) ? field.value.map(String) : []}
                                  ...
                                  onValueChange={handleValueChange(field.onChange, 'subGroups')}
                                  externalLabels={productLabels?.subGroups ?? []}
                                  defaultValue={Array.isArray(field.value) ? field.value.map(String) : []}
                            ...
                            return (
                              <FormItem>
                                ...
                                value={Array.isArray(field.value) ? field.value.map(String) : []}
                                  ...
                                  onValueChange={handleValueChange(field.onChange, 'subSubGroups')}
                                  externalLabels={productLabels?.subSubGroups ?? []}
                                  defaultValue={Array.isArray(field.value) ? field.value.map(String) : []}
                            ...
                            return (
                              <FormItem>
                                ...
                                value={Array.isArray(field.value) ? field.value.map(String) : []}
                                  ...
                                  onValueChange={handleValueChange(field.onChange, 'managerAuto')}
                                  externalLabels={productLabels?.managerAuto ?? []}
                                  defaultValue={Array.isArray(field.value) ? field.value.map(String) : []}
                      ...
                      <CardContent className="flex flex-col gap-3">
                        <FormField
                          ...
                          render={({field}) => {
                            ...
                            return (
                              <FormItem>
                                <FormLabel className={hasError ? "text-destructive" : ""}>Структурное подразделение</FormLabel>
                                <FormControl>
                                <MultiSelect
                                maxCount={1}
                                value={Array.isArray(field.value) ? field.value.map(String) : []}
                                  ...
                                  onValueChange={handleValueChange(field.onChange, 'subDivisionProducts')}
                                  externalLabels={productLabels?.subDivisionProducts ?? []}
                                  defaultValue={Array.isArray(field.value) ? field.value.map(String) : []}
          ...
          <p className="text-[12px] text-destructive">Обязательно для заполнения</p>
                        ...
                        <FormField
                          ...
                          render={({field}) => {
                            ...
                            return (
                              <FormItem>
                                <FormLabel  className={hasError ? "text-destructive" : ""}>Команда</FormLabel>
                                <FormControl>
                                <MultiSelect
                                maxCount={1}
                                value={Array.isArray(field.value) ? field.value.map(String) : []}
                                  ...
                                  onValueChange={handleValueChange(field.onChange, 'teamProducts')}
                                  externalLabels={productLabels?.teamProducts ?? []}
                                  defaultValue={Array.isArray(field.value) ? field.value.map(String) : []}
          ...
          <p className="text-[12px] text-destructive">Обязательно для заполнения</p>
                        ...
                        <FormField
                          ...
                          render={({field}) => {
                            ...
                            return (
                              <FormItem>
                                <FormLabel  className={hasError ? "text-destructive" : ""}>Направление</FormLabel>
                                <FormControl>
                                <MultiSelect
                                maxCount={1}
                                value={Array.isArray(field.value) ? field.value.map(String) : []}
                                  ...
                                  onValueChange={handleValueChange(field.onChange, 'directionProducts')}
                                  externalLabels={productLabels?.directionProducts ?? []}
                                  defaultValue={Array.isArray(field.value) ? field.value.map(String) : []}
          ...
          <p className="text-[12px] text-destructive">Обязательно для заполнения</p>
                        ...
                        <FormField
                          ...
                          render={({field}) => {
                            ...
                            return (
                              <FormItem>
                                <FormLabel  className={hasError ? "text-destructive" : ""}>Тип поставщика</FormLabel>
                                <FormControl>
                                <MultiSelect
                                maxCount={1}
                                value={Array.isArray(field.value) ? field.value.map(String) : []}
                                  ...
                                  onValueChange={handleValueChange(field.onChange, 'typeProducts')}
                                  externalLabels={productLabels?.typeProducts ?? []}
                                  defaultValue={Array.isArray(field.value) ? field.value.map(String) : []}
          ...
          <p className="text-[12px] text-destructive">Обязательно для заполнения</p>
                        ...
                        <FormField
                          ...
                          render={({field}) => {
                            ...
                            return (
                              <FormItem>
                                <FormLabel className={hasError ? "text-destructive" : ""}>Сезон</FormLabel>
                                <FormControl>
                                <MultiSelect
                                maxCount={1}
                                value={Array.isArray(field.value) ? field.value.map(String) : []}
                                  ...
                                  onValueChange={handleValueChange(field.onChange, 'seasonalityProducts')}
                                  externalLabels={productLabels?.seasonalityProducts ?? []}
                                  defaultValue={Array.isArray(field.value) ? field.value.map(String) : []}
          ...
          <p className="text-[12px] text-destructive">Обязательно для заполнения</p>
                        ...
                        <FormField
                          ...
                          render={({field}) => {
                            ...
                            return (
                              <FormItem>
                                <FormLabel className={hasError ? "text-destructive" : ""}>Справочник экономиста</FormLabel>
                                <FormControl>
                                <MultiSelect
                                maxCount={1}
                                value={Array.isArray(field.value) ? field.value.map(String) : []}
                                  ...
                                  onValueChange={handleValueChange(field.onChange, 'groupsEconomist')}
                                  externalLabels={productLabels?.groupsEconomist ?? []}
                                  defaultValue={Array.isArray(field.value) ? field.value.map(String) : []}
          ...
          <p className="text-[12px] text-destructive">Обязательно для заполнения</p>
                  ...
                  <Card className="bg-background">
                    ...
                    <Button 
                        variant="outline" 
                      > Отмена </Button>
                      <Button disabled={!form.formState.isValid} loading={isUpdateLoading}>Сохранить</Button>
                    </CardContent>
                  </Card>
                </form>
              </Form>
