...
import { useDownloadForestController } from "../model/api/controller";
import {
  Dialog,
  DialogTrigger,
  DialogContent,
...
const DownloadForest = ({ rows, tab }: { rows: number; tab: string }) => {
  const { downloadForest, downloadForestWriteOff } =
    useDownloadForestController();
  const { getApiPayload } = useFiltersStore();
  const [isOpen, setIsOpen] = useState(false);
  const payload = getApiPayload();

  const handleDownloadForest = async (typeFile: "csv" | "excel") => {
    if (tab === "write-off") {
      await downloadForestWriteOff({
        ...payload,
      ...
      <DialogContent>
        ...
        <DialogFooter className="flex flex-row gap-2 w-full">
          <Button
            ...
            onClick={() => handleDownloadForest("excel")}
          >
            Excel
          </Button>
          <Button
            ...
            onClick={() => handleDownloadForest("csv")}
          >
            CSV
          </Button>
        </DialogFooter>
