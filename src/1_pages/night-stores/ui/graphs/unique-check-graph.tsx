...
interface UniqueCheckGraphProps {
  isLoading: boolean;
  graph: BarGraphResponse["data"] | null;
}

const UniqueCheckGraph = ({ isLoading, graph }: UniqueCheckGraphProps) => {
                  ...
                  return params
                    .map(
                      (item) =>
                        ...
                        <strong>Чеки:</strong> ${item.value.toLocaleString().replace(/,/g, " ")} шт.`,
                    )
                    .join("<br/>");
                }
...
export default UniqueCheckGraph;
