...
export const useSubsubgroup = () => {
  ...
  const handleOpenSubsubgroupsSelect = async (isOpen: boolean) => {
    ...
    try {
      const response = await getSubSubGroups({product: defaultValues});
      const apiOptions = response.map((franchise: any) => ({
        label: franchise.sub_sub_groups,
        value: String(franchise.id_sub_sub_groups || "")
      }));
