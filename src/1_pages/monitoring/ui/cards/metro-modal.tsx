...
interface MetroModalProps {
  product: MetroProduct | null;
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

export const MetroModal = ({
  product,
  open,
  onOpenChange,
}: MetroModalProps) => {
  ...
  // Парсим packing
  let packing: any = null;
  try {
    packing = JSON.parse(product.packing);
  } catch {
            ...
            <div className="flex-1 flex flex-col gap-4">
              ...
              <div>
                ...
                <div className="text-3xl font-bold text-primary">
                  {prices?.price ? prices.price.toLocaleString() : "Не указана"}{" "}
                  ₽
                </div>
                {prices?.old_price && (
                  <div className="flex items-center gap-1 text-sm text-muted-foreground mt-1">
                    <span className="line-through">
                      {prices.old_price.toFixed(2)} ₽
                    </span>
                  </div>
                )}
                {prices?.is_promo && (
                  <Badge variant="default" className="bg-orange-500 mt-2 w-fit">
                    Акция
                  </Badge>
                )}
              </div>
