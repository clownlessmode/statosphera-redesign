...
function TabsList({
  ...
  return (
    <TabsPrimitive.List
      data-slot="tabs-list"
      className={cn(
        "bg-muted text-muted-foreground inline-flex h-9 w-fit items-center justify-center rounded-lg md:p-[3px]",
        className,
      )}
      {...props}
    />
  ...
  return (
    <TabsPrimitive.Trigger
      ...
      className={cn(
        ...
        isMobile
          ? "md:rounded-none h-full p-2.5"
          : "rounded-lg h-[calc(100%-1px)]",
        className,
      )}
      {...props}
    />
